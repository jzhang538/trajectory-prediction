...
10: Width
11: Lateral maneuver
12: Longitudinal maneuver
13-63: Car Ids at grid location
64: Num_nbrs
65-: Neighbor Car Ids
%}
...
for k = 1:6
    traj{k} = traj{k}(:,[1,2,3,6,7,15,13,14,10,11]);
    if k <=3
...
for ii = 1:6
    ...
    for k = 1:length(traj{ii}(:,1))        
        ...
        % Get grid locations and Nbr Vehicles:
        num_nbrs = 0;
        dis = 120;
...
for k = 1:6
    sp1 = round(0.7*max(trajAll(trajAll(:,1)==k,2)));
    sp2 = round(0.8*max(trajAll(trajAll(:,1)==k,2)));
    sp3 = round(0.7*max(trajAll(trajAll(:,1)==k,3)));
    sp4 = round(0.8*max(trajAll(trajAll(:,1)==k,3)));
    disp("Split")
    disp(sp1)
    disp(sp2)
    disp(sp3)
    ...
    trajTr = [trajTr;trajAll(trajAll(:,1)==k & trajAll(:,3)<=ul1, :)];
    trajVal = [trajVal;trajAll(trajAll(:,1)==k & trajAll(:,3)>ul1 & trajAll(:,3)<=ul2, :)];
    trajTs = [trajTs;trajAll(trajAll(:,1)==k & trajAll(:,3)>ul2, :)];
end
...
for k = 1:6
    ...
    trajAllSet = trajAll(trajAll(:,1)==k,:);

    carIds = unique(trajSet(:,2));
    for l = 1:length(carIds)
        vehtrack = trajAllSet(trajAllSet(:,2) ==carIds(l),[3,4,5,6,7,8])';
...
for k = 1: size(trajTr,1)
    t = trajTr(k,3);
    if tracksTr{trajTr(k,1),trajTr(k,2)}(1,end) <= 100
        continue;
    end
...
for k = 1: size(trajVal,1)
    t = trajVal(k,3);
    if tracksVal{trajVal(k,1),trajVal(k,2)}(1,end) <= 100
        continue;
    end
...
for k = 1: size(trajTs,1)
    t = trajTs(k,3);
    if tracksTs{trajTs(k,1),trajTs(k,2)}(1,end) <= 100
        continue;
    end
